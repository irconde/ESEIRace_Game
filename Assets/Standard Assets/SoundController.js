...
static var backgroundMusic : AudioSource = null;
...
public var effectsPlaying : boolean = true;

// Create the needed AudioSources
function Start()						//Awake
{
	/*
	var src = new StreamReader("./MusicVolume.txt");
	var line  = src.ReadLine();
	//Debug.Log(line);
	BackgroundMusicVolume = parseFloat(line);
	src.Close();
	
	src = new StreamReader("./FXVolume.txt");
	line  = src.ReadLine();
	//Debug.Log(line);
	FXVolume = parseFloat(line);
	src.Close();
...
function Update()
{
	...
	//KAudio.volume = Mathf.Lerp(0, KVolume, carSpeedFactor); 				//IVAN
    ...
    DAudio.volume = FXVolume;
	EAudio.volume = FXVolume;
	FAudio.volume = FXVolume;
	KAudio.volume = FXVolume;
	LAudio.volume = FXVolume;

	windAudio.volume = FXVolume;
	carAudio.volume = FXVolume;
